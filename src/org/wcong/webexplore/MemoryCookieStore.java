 ...
 */
public class MemoryCookieStore implements CookieStore, Serializable {

    private void writeObject(ObjectOutputStream stream) throws IOException {
        if (uriIndex != null) {
            List<MiniCookie> cookieList = new ArrayList<>(uriIndex.size());
            for (Map.Entry<URI, List<HttpCookie>> entry : uriIndex.entrySet()) {
                URI uri = entry.getKey();
                String url = uri.toString();
                List<HttpCookie> httpCookieList = entry.getValue();
                for (HttpCookie httpCookie : httpCookieList) {
                    MiniCookie miniCookie = new MiniCookie();
                    miniCookie.setName(httpCookie.getName());
                    miniCookie.setValue(httpCookie.getValue());
                    miniCookie.setUrl(url);
                    cookieList.add(miniCookie);
                }
            }
            stream.writeObject(cookieList);
        }
    }
    ...
    private void readObject(ObjectInputStream stream)
        ...
        List<MiniCookie> cookieList = (List<MiniCookie>) stream.readObject();
        for (MiniCookie miniCookie : cookieList) {
            add(new URI(miniCookie.getUrl()), new HttpCookie(miniCookie.getName(), miniCookie.getValue()));
        }
    }
